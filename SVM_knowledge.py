...
# -*- coding: utf-8 -*-
"""
Created on Friday May 3 23:00:12 2019
...
from load_knowledge_data import load_data
...
#    pyplot.title("The structure of the disease classes")
#    pyplot.show()
    '''----------------------------------------------------'''
...
def cross_validation(X, Y, k=10):
    ...
    for train_index, test_index in kf.split(X, Y):
        ...
        cross_model = SVC(C=100.0, cache_size=200, class_weight=None, coef0=0.0,
             decision_function_shape='ovr', degree=3, gamma='auto',
...
#    pyplot.show()
    ...
    model2 = SVC(C=100.0, cache_size=200, class_weight=None, coef0=0.0,
      decision_function_shape='ovr', degree=3, gamma='auto',
      kernel='rbf', max_iter=-1, probability=False, random_state=None,
      shrinking=True, tol=0.001, verbose=False)
...
def sample_test_comparison():
    ...
    score2 = KNN_knowledge.KNN_phase1(x_train, x_test, y_train, y_test)
    for s in sample_size:
        ...
        for i in range(5):
            ...
            try:
                p2 += KNN_knowledge.KNN_phase2(int(s*n), x_train, x_test, y_train, y_test)
                count2 += 1
            except:
                p2 += 0
    ...
    pyplot.hlines(score1, 0.1, 0.5, color='purple')
    pyplot.hlines(score2, 0.1, 0.5, color='red')
